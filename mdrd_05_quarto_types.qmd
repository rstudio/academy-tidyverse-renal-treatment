...
1. Create two new columns, `diet` and `bp`, by separating the character strings in `treatment`. (*Hint* Look up the help page for `tidyr::separate()`)
...
#| label: compare
#| eval: false
waldo::compare(treatments, solution, tolerance = 1e-4, ignore_attr = .groups)
```
